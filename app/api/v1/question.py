...
from app.schemas.answer import AnswerListResponse, AnswerResponse
from app.schemas.http_response import ErrorResponse
from app.services import question_service, answer_service
...
def get_answers_by_question(
    ...
    filter_key_1: Optional[str] = Query(
        None,
        alias="filter-key-1",
        description="First filter key (e.g., content, is_correct)",
    ),
    filter_value_1: Optional[str] = Query(
        None, alias="filter-value-1", description="First filter value"
    ),
    filter_key_2: Optional[str] = Query(
        None, alias="filter-key-2", description="Second filter key"
    ),
    ...
    **Examples:**
    ...
    """
    request_params = dict(request.query_params)
    answers, total = answer_service.get_answers_by_question_id(
        db,
        question_id,
        search_key=key,
        search_value=value,
...
def get_answer_by_id(
    ...
    This endpoint does not require authentication.
    """
    answer = answer_service.get_answer_by_id(db, question_id, answer_id)
