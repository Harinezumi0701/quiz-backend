...
from app.schemas.auth import RegisterRequest, LoginRequest, TokenData, RefreshTokenRequest, RevokeTokenRequest
...
def refresh_access_token(db: Session, request: RefreshTokenRequest) -> TokenData:
    ...
    if not refresh_token:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=ERROR_INVALID_REFRESH_TOKEN
        )
    ...
    if refresh_token.expires_at < datetime.now(timezone.utc):
        ...
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=ERROR_REFRESH_TOKEN_EXPIRED
        )
